...
static void initCompiler(Compiler* compiler, FunctionType type) {
  ...
  // When compiling a function declaration, we call
  // initCompiler() right after we parse the function’s
  // name. That means we can grab the name right then
...
static void classDeclaration() {
  ...
  if (match(TOKEN_LESS)) {
    consume(TOKEN_IDENTIFIER, "Expect a superclass name.");
    variable(false);
